...
export function WorldControls({loading,busy,onEnter,onSkip}:Props){
 return <>
  <div data-lobby-chrome className="pointer-events-none absolute left-6 top-6 z-20 text-[#183743]">
  ...
  <div data-lobby-chrome className="absolute bottom-6 right-6 z-20"><button className={button} disabled={busy} onClick={(event) => loading || event.detail === 0 ? onSkip() : onEnter()}>{loading?'Skip to portfolio':'Enter portfolio'}<span aria-hidden>↗</span></button></div>
