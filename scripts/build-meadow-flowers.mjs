/** CC0 Quaternius Stylized Nature MegaKit. Source IDs are recorded in assets/lobby-world/meadow-sources.json. */
...
const io=new NodeIO(),result=new Document(),world=result.createScene('Meadow flowers');
const heights={flowers:.42,'flower-bush':.8};
...
await result.transform(unpartition(),dedup());await io.write('/tmp/meadow-flowers.glb',result);
execFileSync('pnpm',['exec','gltf-transform','optimize','/tmp/meadow-flowers.glb','public/lobby/world/meadow-flowers.glb','--compress','meshopt','--texture-compress','webp','--texture-size','512','--simplify','true','--simplify-ratio','.75','--simplify-error','.02','--join','false','--flatten','false'],{stdio:'inherit'});
