...
### Reference houses — September 2026

`reference-houses.glb` replaces the generic frontage and adds franchise easter eggs. Every reference model is authored with its entrance on +Z, and `facingDesk()` turns each one toward the desk so the fronts read from the seated view. Landmark spots are spread across the frame and were chosen from a rendered visibility pass (colour-coded markers on a terrain grid, decoded from a screenshot) so none hides behind the canopy: Ichiraku Ramen near the desk left of the road, the pineapple on the open plateau, Kame House mid-river, the Pokémon Center on the far right bank below the arch, hobbit holes and small timber-house groups on the far slopes and through the arch opening. `lib/lobby/settlement-placements.ts` is the single layout shared by `valley-village.tsx` (Quaternius fillers) and `reference-houses.tsx` (textured instances). Sources are the owner's Sketchfab "autoconverted glTF" downloads listed in `reference-house-sources.json`; place the zips in one directory and run `node scripts/build-reference-houses.mjs <directory>`, then `pnpm assets:version`. Per-model `size`, `sizeAxis`, `yaw`, `triangleBudget` and `dropNodes` (regex of scene-dressing nodes to remove) are tuned in the manifest.
